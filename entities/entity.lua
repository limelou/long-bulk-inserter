local hit_effects = require("__base__.prototypes.entity.hit-effects")
local sounds = require("__base__.prototypes.entity.sounds")

data:extend({
  {
    type = "inserter",
    name = "long-handed-bulk-inserter",
    icon = "__Long-handed_Bulk__/graphics/icon.png",
    flags = {"placeable-neutral", "placeable-player", "player-creation"},
	icon_draw_specification = {scale = 0.5},
	
    bulk = true,
	minable = { mining_time = 0.1, result = "long-handed-bulk-inserter" },
	max_health = 160,
    filter_count = 5,

    corpse = "lhb-inserter-remnants",
	dying_explosion = "lhb-inserter-explosion",
    resistances = {
      { type = "fire", percent = 90 }
    },
	damaged_trigger_effect = hit_effects.entity(),

    extension_speed = 0.05,
    rotation_speed = 0.03,
	starting_distance = 1.7,
	hand_size = 1.5,

    collision_box = {{-0.15, -0.15}, {0.15, 0.15}},
    selection_box = {{-0.4, -0.35}, {0.4, 0.45}},
    pickup_position = {0, -2},
    insert_position = {0, 2.2},

    energy_source = {
		type = "electric",
		usage_priority = "secondary-input",
		drain = "1kW"
	},
	energy_per_movement = "28kJ",
    energy_per_rotation = "28kJ",
	
    fast_replaceable_group = "long-handed-inserter",
	open_sound = sounds.inserter_open,
    close_sound = sounds.inserter_close,
    working_sound = sounds.inserter_fast,
	
	impact_category = "metal",
	
	hand_base_picture =
    {
      filename = "__Long-handed_Bulk__/graphics/inserter/inserter-hand-base.png",
      priority = "extra-high",
      width = 32,
      height = 136,
      scale = 0.25
    },
    hand_closed_picture =
    {
      filename = "__Long-handed_Bulk__/graphics/inserter/inserter-hand-closed.png",
      priority = "extra-high",
      width = 100,
      height = 164,
      scale = 0.25
    },
    hand_open_picture =
    {
      filename = "__Long-handed_Bulk__/graphics/inserter/inserter-hand-open.png",
      priority = "extra-high",
      width = 130,
      height = 164,
      scale = 0.25
    },
    hand_base_shadow =
    {
      filename = "__base__/graphics/entity/burner-inserter/burner-inserter-hand-base-shadow.png",
      priority = "extra-high",
      width = 32,
      height = 132,
      scale = 0.25
    },
    hand_closed_shadow =
    {
      filename = "__base__/graphics/entity/bulk-inserter/bulk-inserter-hand-closed-shadow.png",
      priority = "extra-high",
      width = 100,
      height = 164,
      scale = 0.25
    },
    hand_open_shadow =
    {
      filename = "__base__/graphics/entity/bulk-inserter/bulk-inserter-hand-open-shadow.png",
      priority = "extra-high",
      width = 130,
      height = 164,
      scale = 0.25
    },
    platform_picture =
    {
      sheet =
      {
        filename = "__Long-handed_Bulk__/graphics/inserter/inserter-platform.png",
        priority = "extra-high",
        width = 105,
        height = 79,
        shift = util.by_pixel(1.5, 7.5-1),
        scale = 0.5
      }
    },
    circuit_connector = circuit_connector_definitions["inserter"],
    circuit_wire_max_distance = inserter_circuit_wire_max_distance,
    default_stack_control_input_signal = inserter_default_stack_control_input_signal
  }
})
